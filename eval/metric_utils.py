...
def volume_sum_update(volume_sum, pcd, x_range, y_range, voxel_size,min_bound):
    # mask out invalid points
    mask_x = np.logical_and(pcd[:, 0] > x_range[0], pcd[:, 0] < x_range[1])
    mask_y = np.logical_and(pcd[:, 1] > y_range[0], pcd[:, 1] < y_range[1])
    mask = mask_x & mask_y
...
def voxelize_pcd(pcd,x_range, y_range, voxel_size, min_bound, vol_shape):
    # mask out invalid points
    mask_x = np.logical_and(pcd[:, 0] > x_range[0], pcd[:, 0] < x_range[1])
    mask_y = np.logical_and(pcd[:, 1] > y_range[0], pcd[:, 1] < y_range[1])
    mask = mask_x & mask_y
...
def compute_logits(data_type, modality, *args):
    ...
    assert modality in ['range', 'voxel', 'point_voxel','rangevit','loc']
    ...
    for data in args:
        ...
        for i in range(math.ceil(len(data) / bs)):
            ...
            batch_out = compute_batch_logits(batch,model, is_voxel, dataset_config)
            if is_voxel:
                all_logits_list.extend(batch_out)
            else:
                all_logits_list.append(np.vstack(batch_out))
        if is_voxel:
...
def compute_batch_logits(batch,model,is_voxel, dataset_config):
    if is_voxel:
        ...
        with torch.no_grad():
            ...
            return batch2list(batch_out, AGG_TYPE, **dataset_config)
    else:
        with torch.no_grad():
            return model(batch)
