...
class SiT(nn.Module):
    ...
    def forward(self, x, y, zs, loss_kwargs, time_input=None, noises=None):
        ...
        #check if there is nan in x and don't update bn stats

        # replaces NaN→0, +Inf→max, -Inf→min
        x_no_nan = torch.nan_to_num(x)   
        normalized_x = self.bn(x_no_nan)
        ...
        for i, block in enumerate(self.blocks):
            ...
            if (i + 1) == self.encoder_depth:
                ...
                if self.upsample_factor > 1:
                    h, w = self.x_embedder.grid_size
                    interpolated_zs_tilde = []
                    for z_tilde in zs_tilde:
                        # Reshape from (N, T, feature_dim) to (N, feature_dim, H, W)
                        feature_dim = z_tilde.shape[-1]
                        z_tilde_2d = z_tilde.permute(0, 2, 1).reshape(N, feature_dim, h, w)
                        
                        # Apply bilinear interpolation
                        z_tilde_upsampled = F.interpolate(
                            z_tilde_2d, 
                        ...
                        # Reshape back to (N, T_new, feature_dim)
                        h_new, w_new = z_tilde_upsampled.shape[-2:]
                        z_tilde_flat = z_tilde_upsampled.reshape(N, feature_dim, -1).permute(0, 2, 1)
                        interpolated_zs_tilde.append(z_tilde_flat)
                    
                    zs_tilde = interpolated_zs_tilde
                
                # NOTE: add a shortcut for feature extraction
                if loss_kwargs["align_only"]:
                    break
        x = self.final_layer(x, c)                            # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)#,shape=model_target.shape[-2:])                                # (N, out_channels, H, W)

        # loss computation
        denoising_loss = None if loss_kwargs["align_only"] else mean_flat((x - model_target) ** 2)

        proj_loss = torch.zeros(zs[0].shape[0], device=x.device)
    ...
    def inference_cond_feats(self, x, t, y, zs, v_feat_scale, mask=None):
        x_init = x.clone().detach().requires_grad_(True)
        x = self.x_embedder(x_init) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        N, T, D = x.shape
        if mask is None:
            mask = torch.ones((N,T), device=x.device, dtype=torch.bool)
        ...
        for i, (z, z_tilde) in enumerate(zip(zs, zs_tilde)):
            z_tilde = torch.nn.functional.normalize(z_tilde[mask[:,:]], dim=-1) 
            z = torch.nn.functional.normalize(z[mask[:,:]], dim=-1) 
            proj_loss += mean_flat(-(z * z_tilde).sum(dim=-1))
        proj_loss /= (len(zs))
        ...
        grad = torch.autograd.grad(
                outputs=proj_loss,
                inputs=x_init,
                grad_outputs=torch.ones_like(proj_loss),
                create_graph=False,
                retain_graph=False
            )[0]        
        v_feat = grad * v_feat_scale
        torch.cuda.empty_cache()
        return x , v_feat, proj_loss

    def forward_feats(self, x, t, y, depth=None):
        if depth is None:
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, c)                      # (N, T, D)
            if (i + 1) == depth:
                return [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
...
def SiT_S_1(**kwargs):
    return SiT(depth=12, hidden_size=384, decoder_hidden_size=384, patch_size=1, num_heads=6, **kwargs)
