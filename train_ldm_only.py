...
def main(args):    
    ...
    if args.enc_type != None:
        # Only load encoders on the main process to avoid race conditions
        if accelerator.is_main_process:
        ...
        # Now all non-main processes can safely load the encoders
    ...
    upsample_factor = 2 if args.model.startswith('UNet') else int(args.model[-1])//2 
    ...
    # Load VAE checkpoint with proper multi-GPU handling
    if accelerator.is_main_process:
        vae_ckpt = torch.load(args.vae_ckpt, map_location='cpu')
        vae.load_state_dict(vae_ckpt)
        del vae_ckpt
    
    # Wait for main process to finish loading, then broadcast to other processes
    accelerator.wait_for_everyone()
    
    # For non-main processes, load the checkpoint after main process is done
    if not accelerator.is_main_process:
        vae_ckpt = torch.load(args.vae_ckpt, map_location='cpu')
        vae.load_state_dict(vae_ckpt)
        del vae_ckpt
    ...
    for epoch in range(args.epochs):
        for raw_image, latents, y in train_dataloader:
            ...
            if args.unconditional:
                # For unconditional training, we use a dummy label
                y = torch.zeros(raw_image.size(0), dtype=torch.long, device=device)
            labels = y.to(device)
